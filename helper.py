...
try:
    ...
    os.system("python3 -m pip install pyromod") #pyromodded

#setup
id_users = [1264694450] # for access
id_owners = [1264694450] # for access
app = Client('ZERO_SELF', api_hash='your_hash', api_id=000000,
             bot_token='your_bot_token')
# turn ON inline mode robot in @BotFather
...
#user id
users = filters.user(id_users) 
owners = filters.user(id_owners) 

@app.on_inline_query()
def answer(client, inline_query):
    if inline_query.from_user.id in id_users:
      inline_query.answer(
          results=[
              InlineQueryResultArticle(
                  title="Helper",
                  input_message_content=InputTextMessageContent(
                      f"__Hello {inline_query.from_user.first_name}\n Welcome To Z E R O - SELF"
                  ),
                  description="Z E R O - SELF Self Panel",
                  reply_markup=mark
              )
          ],
          cache_time=1
...
@app.on_callback_query(~users)
def test(app, call): 
  call.answer("Zero Self", show_alert=True)
    
@app.on_message(filters.private&owners&filters.command("panel"), group=-1)
async def updates(app, m:Message):
     await app.send_message(m.chat.id, "**Zero Owner**", reply_markup=keyboard_idk)
...
async def updates(app, m:Message):
     ...
     await app.send_message(id_owners[0], f"✅ User {kos} Started The Bot ✅")
...
async def updates(app, m:Message):
 ...
 if text == "Add User":
   try:
     ...
     id_users.append(int(answer.text))
   ...
   if int(answer.text) in users:
     try:
       num = id_users.index(int(answer.text))
       id_users.remove(id_users[num])
   ...
   try:
     ...
     op = 1
     if len(id_users) >= 1:
       for i in range(0,int(len(id_users))):
         s += f"֍ {op} -> `{id_users[i]}`\n"
         op += 1
   ...
   answer = await app.ask(m.chat.id, '**Send Me User ID**:')
   try:
     if int(answer.text) in id_users:
       id_owners.append(int(answer.text))
   ...
   if int(answer.text) in id_users:
     try:
       num = id_owners.index(int(answer.text))
       id_owners.remove(id_owners[num])
   ...
   try:
     ...
     op = 1
     if len(id_owners) >= 1:
       for i in range(0,int(len(id_owners))):
         s += f"֍ {op} -> `{id_owners[i]}`\n"
         op += 1
